...
class FirstDoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, slice, dropout):
...
class UNET21D(nn.Module):
    def __init__(
            self, in_channels=1, out_channels=1, features=[64, 128, 256, 512], slice=1, dropout=0.0
    ):
        ...
        for feature in features:
            ...
            if count==0 and slice>0: 
                ...
                self.downs.append(FirstDoubleConv(in_channels, feature, slice, dropout=0))
                count+=1
                in_channels = feature
            else:
                self.downs.append(DoubleConv(in_channels, feature, dropout=0))
                in_channels = feature
        ...
        self.bottleneck = DoubleConv(features[-1], features[-1]*2, dropout=0)
